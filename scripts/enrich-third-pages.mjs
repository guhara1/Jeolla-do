...
const cityMemo = {
  "광주": {
    ...
    check: ["건물명", "주차 가능 여부", "출입 방식", "상무·첨단·수완권 중 실제 방문 지점"]
  },
  "전주": {
    ...
    check: ["숙소명", "동 이름", "출입 방식", "완산권·덕진권 구분"]
  },
  "완주": {
    ...
    check: ["읍면명", "상세 주소", "차량 진입 여부", "추가 이동 조건"]
  },
  "군산": {
    ...
    check: ["주차 가능 여부", "출입 방식", "숙소·주거지 구분", "심야 이동 가능 시간"]
  },
  "여수": {
    ...
    check: ["숙박지명", "객실 출입 조건", "희망 시간대", "관광지 주변 도로 상황"]
  },
  "순천": {
    ...
    check: ["아파트·숙소 출입 방식", "연락 가능한 시간", "조례권·원도심권 구분", "방문 지점"]
  },
  "광양": {
    ...
    check: ["중마권·광양읍권 구분", "외곽 주소", "산단 주변 이동 가능 시간", "추가 이동 조건"]
  }
};
...
function hashText(text) { return [...text].reduce((sum, char, index) => sum + char.charCodeAt(0) * (index + 11), 0); }
function pick(list, seed, offset = 0) {
  const fallback = ["상담 전 세부 조건 확인"];
  const target = Array.isArray(list) && list.length ? list : fallback;
  return target[Math.abs(seed + offset * 17) % target.length];
}
function profile(city) { return cityMemo[city] || cityMemo["광주"]; }
function districtText(district) { return districtMemo[district] || "세부 생활권에 따라 이동 시간과 방문 조건이 달라질 수 있습니다"; }
...
function ul(items) { return `<ul>${items.map((item) => `<li>${item}</li>`).join("")}</ul>`; }
function card(title, body, items) { return `<article class="detail-card"><h2>${title}</h2><p>${body}</p>${ul(items)}</article>`; }
function intro(city, district, dong, seed) {
  ...
  return [
    ...
    `예약 전에는 ${pick(feeItems, seed, 6)}, ${pick(feeItems, seed, 7)}, ${pick(p.check, seed, 8)}을 함께 확인하는 것이 좋습니다.`,
